...
device_class:
  ...
  type: string
expire_after:
  description: "Defines the number of seconds after the value expires if it's not updated. After expiry, the value is clearedk, and the availability is set to false"
  required: false
  type: int
value_template:
